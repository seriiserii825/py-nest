...
# typeorm-ts-node-esm assumes "type": "module" in package.json; use
# typeorm-ts-node-commonjs instead for CommonJS projects.
PACKAGE_JSON_MIGRATIONS = """\
{
  "scripts": {
    "migration:check": "typeorm-ts-node-esm migration:generate -d src/data-source.ts src/migrations/TempCheck && echo 'Changes detected!' || echo 'No changes'",
    "migration:to_check": "npx typeorm-ts-node-esm schema:log -d src/data-source.ts",
